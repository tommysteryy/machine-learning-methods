    ...
    def fit(self, X, y):
        ...
        while not np.all(selected):
            ...
            for j in range(d):
                ...
                X_subfeatures = X[:,selected_with_j]
                X_sub_n, X_sub_d = X_subfeatures.shape
    
                w_init_guess = np.zeros(X_sub_d)
                w_opt, *_ = self.optimize(w_init_guess, X_subfeatures, y)

                local_loss, *_ = self.loss_fn.evaluate(w_opt, X_subfeatures, y)
...
class LogRegClassifierOneVsAll(LogRegClassifier):
    ...
    def fit(self, X, y):
        ...
        y_each_class = np.zeros_like(y)
        ## Train each class separately
        for c in range(k):
            ## Label all non-c class as -1, all c-class as 1
            examples_of_class_c = y == c
            y_each_class[examples_of_class_c] = 1
            y_each_class[~examples_of_class_c] = -1

            ## LogisticRegressionOptimize on it
            w_c = np.zeros(d)
            w_c_opt, *_ = self.optimize(w_c, X, y_each_class)

            ## Store that w_c into the matrix at row c
            W[c:c+1] = w_c_opt
...
class MulticlassLogRegClassifier(LogRegClassifier):
    ...
    def fit(self, X, y):
        ...
        # quick check that loss_fn is implemented correctly
        W_init = np.zeros([k, d])
        W_init = W_init.reshape(-1)
        self.loss_fn.check_correctness(W_init, X, y)

        W = np.zeros([k,d])
        W = W.reshape(-1)

        W_opt, *_ = self.optimize(W, X, y)
        W_opt = W_opt.reshape((k,d))
